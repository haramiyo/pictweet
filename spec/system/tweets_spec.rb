  ...
  RSpec.describe 'ツイート編集', type: :system do
    ...
    context 'ツイート編集ができるとき' do
      it 'ログインしたユーザーは自分が投稿したツイートの編集ができる' do
        ...
        fill_in 'tweet_text', with: "#{@tweet1.text}+編集した投稿"
        ...
        expect(page).to have_content("#{@tweet1.text}+編集した投稿")
      end
    end
    ...
    context 'ツイート編集ができないとき' do
      it 'ログインしたユーザーは自分以外が投稿したツイートの編集画面には遷移できない' do
        ...
        sing_in(@tweet1.user)
        # ツイート2に「編集」ボタンがない
        expect(all(".more")[0].hover).to have_no_link '編集', href: edit_tweet_path(@tweet1)
      end
  ...
  RSpec.describe 'ツイート削除', type: :system do
    ...
    context 'ツイート削除ができるとき' do
      it 'ログインしたユーザーは自らが投稿したツイートの削除ができる' do
        ...
        expect(page).to have_selector(".content_post[style='background-image: url(#{@tweet1.image});']")
    ...
    context 'ツイート削除ができないとき' do
      it 'ログインしたユーザーは自分以外が投稿したツイートの削除ができない' do
        ...
        sing_in(@tweet1.user)
        # ツイート2に「削除」ボタンが無い
        expect(all(".more")[0].hover).to have_no_link '削除', href: tweet_path(@tweet1)
